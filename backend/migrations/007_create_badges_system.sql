...
-- Creates badges table with predefined achievements and user_badges junction table
...
-- Add badge_count to users table for quick access
ALTER TABLE users ADD COLUMN IF NOT EXISTS badge_count INTEGER DEFAULT 0;

-- Create trigger to update badge_count
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 007 completed: Badges system created with % predefined badges', (SELECT COUNT(*) FROM badges);
END $$;
